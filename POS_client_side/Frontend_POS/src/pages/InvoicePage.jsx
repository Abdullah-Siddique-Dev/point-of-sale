...
const InvoicePage = () => {
  ...
  const columns = [
    ...
    { title: "Creation Date", dataIndex: "createdAt", key: "createdAt", render: (text) => <span>{text?.substring(0, 10)}</span> },
    { title: "Payment Method", dataIndex: "paymentMode", key: "paymentMode" },
    { title: "Total Price", dataIndex: "totalAmount", key: "totalAmount", render: (text) => <span>Rs {text}</span>, sorter: (a, b) => a.totalAmount - b.totalAmount },
    {
      title: "Action", dataIndex: "action", key: "action", width: "100px",
      render: (text, record) => (
        <Button size="small" type="primary" onClick={() => { setIsModalOpen(true); setPrintData(record); }}>Print</Button>
      ),
    },
  ];
        ...
        <div className="p-5">
          {invoices ? (
            <div className="bg-white rounded shadow p-4">
              ...
              <Table dataSource={invoices} columns={columns} bordered pagination={true} scroll={{ x: 1000 }} rowKey="_id" />
            </div>
          ) : (
