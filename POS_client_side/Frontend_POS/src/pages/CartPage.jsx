...
const CartPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: `${Math.random()}`,
      render: (text) => {
        return <span>Rs {text.toFixed(2)}</span>;
      },
              ...
              onClick={() => {
                if (record.quantity === 1) {
                  if (window.confirm("Delete Product?")) {
                    ...
                    message.info("Product successfully removed from cart.");
                  }
                }
                if (record.quantity > 1) {
                  dispatch(decrease(record));
    ...
    },
    {
      ...
      key: `${Math.random()}`,
      render: (text, record) => {
        return <span>Rs {(record.quantity * record.price).toFixed(2)}</span>;
      },
    },
    {
      title: "Action",
        ...
        <div className="flex justify-end mt-4 mb-16">
          <Card className="w-72">
            <div className="flex justify-between">
              <span>Subtotal</span>
              <span>
                Rs {cart.total.toFixed(2) > 0 ? cart.total.toFixed(2) : 0}
              </span>
            </div>
            <div className="flex justify-between my-2">
              ...
              <span className="text-red-600">
                {(cart.total * cart.tax) / 100 > 0
                  ? `+Rs ${((cart.total * cart.tax) / 100).toFixed(2)}`
                  : 0}
              </span>
            </div>
            <div className="flex justify-between">
              ...
              <b>
                {" "}
                Rs {(cart.total + (cart.total * cart.tax) / 100).toFixed(2) > 0
            ...
            <Popconfirm
              ...
              description="Are you sure you want to delete all products?"
              okText="Yes"
              cancelText="No"
