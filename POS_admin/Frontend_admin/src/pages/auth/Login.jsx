  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        // Only allow admin login on admin panel
        if (userData.role !== "admin") {
          message.error("Access Denied! Only admins can access this panel.");
          setLoading(false);
          return;
        }
    ...
    <div style={{ minHeight: "100vh", display: "flex" }}>
      ...
      <div style={{ width: 420, minWidth: 380, padding: "0 48px", display: "flex", flexDirection: "column", justifyContent: "center", position: "relative", background: "#fff" }}>
        ...
        <div style={{ position: "absolute", bottom: 32, left: 0, width: "100%", textAlign: "center", fontSize: 13, color: "#6b7280" }}>
          Go to Cashier Panel:&nbsp;
          <a href="http://localhost:3000/login" style={{ color: "#6366f1" }}>Login as Cashier</a>
        </div>
      </div>
